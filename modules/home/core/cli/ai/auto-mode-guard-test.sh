...
run deny "subshell git push" "$(bash_auto '(git push)' "$PLAIN")"
run deny "spaced subshell push" "$(bash_auto '( git push )' "$PLAIN")"
run deny "chained subshell push" "$(bash_auto 'echo hi && (git push)' "$PLAIN")"
run defer "echo quoting subshell" "$(bash_auto "echo '(git push)'" "$PLAIN")"
...
run deny "push after commit" "$(bash_auto 'git commit -m x && git push' "$AI")"
run deny "send-pack after commit" "$(bash_auto 'git commit -m x; git send-pack origin' "$AI")"
run deny "second commit on main" "$(bash_auto "git commit -m x; git -C $PLAIN commit -m y" "$AI")"
run deny "branch changes before commit" "$(bash_auto 'git switch main && git commit -m x' "$AI")"
run defer "two ai commits" "$(bash_auto 'git commit -m x; git commit -m y' "$AI")"
